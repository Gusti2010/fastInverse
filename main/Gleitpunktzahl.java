...
public class Gleitpunktzahl {
     ...
     */
    private static int sizeMantisse = 23;
     ...
     * (-1)^vorzeichen * 1,r_t-2 r_t-3 ... r_1 r_0 * 2^exponent. Die 0 wird zu
    ...
    public void normalisiere() {
        ...
        // finde die Anzahl an führeneden Nullen in der Mantisse
        int numberOfLeadingZeros = Integer.numberOfLeadingZeros(mantisse);

        int bitsToShift = (32 - sizeMantisse) - numberOfLeadingZeros;


        if(bitsToShift > 0) {

            int lastBitSet = 1;
            lastBitSet = mantisse & (1 << (bitsToShift-1));

            mantisse = mantisse >>> bitsToShift;
            exponent += bitsToShift;

            // Runden TODO: Bisher falsch implementiert!!!!
            if(lastBitSet == 1) {
                mantisse += 1;
            }
